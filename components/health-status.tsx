...
export function HealthStatus() {
  ...
  const calculateMemoryUsage = () => {
    if (!healthData || !healthData.services || !healthData.services.memory) {
      return { used: 0, total: 0, percentage: 0 };
    }
    
    const memory = healthData.services.memory;
    const used = parseMemoryString(memory.heapUsed || "0 MB");
    const total = parseMemoryString(memory.heapTotal || "0 MB");
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="flex items-center justify-between p-4 rounded-lg border bg-card">
            ...
            <div className="flex items-center gap-2">
              {getStatusIcon(healthData.services?.redis?.status || "unhealthy")}
              {healthData.services?.redis?.usingFallback && (
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground capitalize">
                  {healthData.services?.mongodb?.connected 
                    ? 'Connected' 
                    : healthData.services?.mongodb?.state || "unknown"}
                </p>
              </div>
            </div>
            {getStatusIcon(healthData.services?.mongodb?.status || "unhealthy")}
          </div>
          ...
          <div className="flex items-center justify-between p-4 rounded-lg border bg-card">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {healthData.services?.memory?.heapUsed || "N/A"}
                </p>
              </div>
            </div>
            {getStatusIcon(healthData.services?.memory?.status || "unhealthy")}
          </div>
        </div>
          ...
          <div className="p-4 rounded-lg border bg-card">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs">
                <span>Used: {healthData.services?.memory?.heapUsed || "N/A"}</span>
                <span>Total: {healthData.services?.memory?.heapTotal || "N/A"}</span>
              </div>
          ...
          <div className="p-4 rounded-lg border bg-card">
            ...
            <div className="text-center">
              <p className="text-2xl font-bold text-orange-600">
                {healthData.services?.redis?.fallbackStorageSize || 0}
              </p>
              <p className="text-xs text-muted-foreground">
          ...
          <div className="p-4 rounded-lg border bg-card">
            ...
            <div className="text-center">
              <p className="text-2xl font-bold text-purple-600">
                {healthData.services?.redis?.connectionRetries || 0}
              </p>
        ...
        <div className="pt-4 border-t">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs text-muted-foreground">
            <div className="flex justify-between">
              ...
              <span className="font-mono">{healthData?.timestamp || "N/A"}</span>
            </div>
            <div className="flex justify-between">
