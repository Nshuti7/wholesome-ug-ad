...
export interface ReusableFormProps<T extends FieldValues> {
  fields: FormField[];
  onSubmit: (data: T) => void | Promise<void> | Promise<boolean> | boolean;
        ...
        {showSuggestions && filteredSuggestions.length > 0 && (
          <div className="absolute z-10 w-full mt-1 bg-popover border border-border rounded-md shadow-lg max-h-60 overflow-auto">
            {filteredSuggestions.map((suggestion) => (
              <div
                key={suggestion}
                className="px-3 py-2 cursor-pointer hover:bg-accent hover:text-accent-foreground text-popover-foreground transition-colors"
...
export function ReusableForm<T extends FieldValues>({
  ...
  // Reset form when defaultValues change (important for edit forms)
  React.useEffect(() => {
    if (defaultValues) {
  ...
  const handleSubmit = async (data: T) => {
    try {
      ...
      // Only reset on successful submission if resetOnSubmit is true
      if (resetOnSubmit && result !== false) {
        form.reset();
      }
      // If onSubmit returns a boolean, we can use it to determine success
      // This is handled by the parent component (FormDialog) for dialog closing
    } catch (error) {
